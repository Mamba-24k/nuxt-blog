<template>
  <div class="mAudio-box d-flex bgCol-more">
    <div class="play-box cur-p">
      <div
        class="rotate-img col-white d-flex ai-center jc-center"
        :class="{'anim-rotate' : isPlay}"
      >
        <i v-show="!isPlay" class="iconfont icon-play" @click="playMusic(true)"></i>
        <i v-show="isPlay" class="iconfont icon-stop" @click="playMusic(false)"></i>
      </div>
    </div>
    <!-- <transition name="slide-show"> -->
    <div v-show="isPlay" class="audio-box d-flex jc-center ai-center flex-1">
      <div class="process-box cur-p mr-2">
      ...
      <audio
        id="music"
        ref="music"
        src="https://www.coco727.com/uploads/name.mp3"
        loop
      ></audio>
    </div>
    <!-- </transition> -->
...
.mAudio-box {
  ...
  .play-box {
    ...
    .rotate-img {
      ...
      font-size: 24px;
      margin: 0 auto;
      background: url("~assets/img/audio.jpg") no-repeat center center/cover;
      .iconfont{
        font-size: 24px;
      }
    }
