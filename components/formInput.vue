  ...
  <div class="fromInput">
    <el-form :model="userForm" :rules="userRules" ref="userForm">
      <el-form-item prop="nickName">
        <el-input size="medium" :maxlength="10" v-model="userForm.nickName" :placeholder="isUpload ? '网站名称' : '起个牛B点的昵称吧～'"></el-input>
      </el-form-item>
      <el-form-item prop="email" v-if="isUpload">
        <el-input size="medium" v-model="userForm.email" :maxlength="20" placeholder="电子邮箱(@xx.com)"></el-input>
      </el-form-item>
      <el-form-item prop="qqEmail" v-else>
        <el-input size="medium" v-model="userForm.qqEmail" :maxlength="20" placeholder="QQ邮箱(仅为了获取QQ头像)"></el-input>
      </el-form-item>
      <el-form-item prop="url">
        <el-input size="medium" v-model="userForm.url" :maxlength="30" placeholder="博客/github地址(开头http://或https://)"></el-input>
      </el-form-item>
      <el-form-item prop="description" v-if="isUpload">
        <el-input size="medium" type="textarea" :maxlength="30" v-model="userForm.description" placeholder="个人描述"></el-input>
      </el-form-item>
      <el-form-item size="medium" prop="avatarImg" v-if="isUpload">
        <el-upload
          class="avatar-uploader cur-p"
          :action="$axios.defaults.baseURL+'/upload'"
          :show-file-list="false"
          :limit='1'
          :on-success="res => $set(userForm, 'avatarImg', res.url)"
        >
          <img v-if="userForm.avatarImg" :src="userForm.avatarImg" class="logo-img" alt="Mamba 曼巴缩略图" />
...
import {validateUrl,validateEmail,validateQQEmail}  from '~/assets/js/validate'
export default {
  props: {
  ...
  data() {
    ...
    return {
      ...
      userRules: {
        nickName: { required: true, message: "必填，且不超过10个字符", trigger: "change" },
        email: { required: true, validator: validateEmail, trigger: "change" },
        qqEmail: { required: true, validator: validateQQEmail, trigger: "change" },
        url: { required: true, message: "必填", trigger: "change" },
        description: { required: true, message: "必填，且不超过30个字符", trigger: "change" },
        avatarImg: { required: true, message: "必填，且仅限jpg/png, 不大于200KB", trigger: "change" },
